...
def test_field_mapping_runs():
    ...
    try:
        result = subprocess.run(
            ['python3', 'physics/particle_field_mapping.py'],
...
def test_scattering_amplitude_runs():
    ...
    try:
        result = subprocess.run(
            ['python3', 'physics/particle_scattering.py'],
            capture_output=True, text=True, check=True,
            env={**os.environ, 'PYTHONPATH': os.environ.get('PYTHONPATH', '') + ':' + '/home/kfm0/.local/lib/python3.12/site-packages'}
        )
...
def test_lagrangian_reduction_runs():
    ...
    try:
        result = subprocess.run(
            ['python3', 'physics/particle_lagrangian.py'],
            capture_output=True, text=True, check=True,
            env={**os.environ, 'PYTHONPATH': os.environ.get('PYTHONPATH', '') + ':' + '/home/kfm0/.local/lib/python3.12/site-packages'}
        )
...
def test_symbolic_sedenion_runs():
    ...
    try:
        result = subprocess.run(
            ['python3', 'physics/particle_symbolic_sedenion.py'],
            capture_output=True, text=True, check=True,
            env={**os.environ, 'PYTHONPATH': os.environ.get('PYTHONPATH', '') + ':' + '/home/kfm0/.local/lib/python3.12/site-packages'}
        )
